...
function UpdateAnimsLib:UpdateConfigSet(fileList)
	...
	assert(trackList.TotalWeight >= 1, "track lists need to have a total weight that is 1 or greater!")
...
function UpdateAnimsLib:UpdateConfig(config)
	coroutine.wrap(function()
		assert(
			typeof(config) == "Instance" or type(config) == "nil",
		...
		if config then
			...
			for _, fileList in pairs(self.AnimData) do
				self:UpdateConfigSet(fileList)
			end
		end
	end)()
